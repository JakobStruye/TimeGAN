...
def batch_generator(data, time, batch_size, b_idx):
  ...
  #return X_mb, T_mb
  X_mb = list(data[i] for i in range(b_idx*batch_size, (b_idx+1)*batch_size))
  T_mb = list(time[i] for i in range(b_idx*batch_size, (b_idx+1)*batch_size))
  return X_mb,T_mb
